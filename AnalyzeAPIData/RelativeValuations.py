...
from config import config_append_summary_statistics
...
def calculate_relative_valuations(market_cap_data_df, earnings_df, ttm_df, balance_sheet_metrics_df):
    ...
    # Clean out the inf values, and do so before the statistics function is applied
    valuations_TTM_df.replace([np.inf, -np.inf], '', inplace=True)

    valuations_TTM_df = config_append_summary_statistics(valuations_TTM_df)
