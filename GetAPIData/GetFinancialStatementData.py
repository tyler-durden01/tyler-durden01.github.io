...
def collect_financial_statement_data(ticker):
    ...
    # Convert columns to numeric for each DataFrame
    ## income_df = income_df.apply(pd.to_numeric, errors='coerce', axis=1)
    ## cash_flow_df = cash_flow_df.apply(pd.to_numeric, errors='coerce', axis=1)
    ...
    financial_statements_df['NOPAT'] = financial_statements_df['operatingIncome'] - financial_statements_df['incomeTaxExpense']
    financial_statements_df['FCFsimple'] = financial_statements_df['EBITDA'] + financial_statements_df['capitalExpenditure']
    financial_statements_df['NWC_simple'] = financial_statements_df['totalCurrentAssets'] - financial_statements_df['totalCurrentLiabilities']
